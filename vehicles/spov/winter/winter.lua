...
Winter plan:
	...
	This only occurs if Winter is digesting in some way.
	...
	Depending on the pills in use,
		...
		Heal
			Belly heals you!
		Raptor laxatives
			Enables disposal animation after digestion			--Piggybacking off of Psyche's pills
...
local swallowLines = {
	"Be sure to squirm for me while your in there.",
	"Oops ate another one!"
}

local releaseLines = {
	"Time for you to come out is it?",
	"I think I've taken all I need from you for now~"
}

local bellyLines = {
	"Don't mind the bones in there that's just from others that didn't make it out."
}

local digestLines = {
	"You're softening up quite nicely in there.",
	"You know what happens next after I digest you right~",
	"Don't mind the bones in there that's just from others that didn't make it out."
}

local digestedLines = {
	"Mine now~",
	"*Urp* I may need another.",
	"Didn't make it out hm?"
}

local disposalLines = {
	"You're softening up quite nicely in there.",
	"You know what happens next after I digest you right~",
	"Going to feel so nice coming back out the back end~"
}
...
function onBegin()
	...
	vsoOnBegin( "state_full", function()
		...
		vsoTimerSet( "gurgle", 2.0, 5 )	--For playing sounds

		vsoTimerSet("speak", 10, 20)
		
		vsoAnimSpeed( 1.0 );
		vsoSay( vsoPick(swallowLines) );
		vsoSound("swallow");
...
function state_full()
	...
	if vsoAnimEnd( "bodyState" ) then
		...
		elseif anim == "fullbig" then
			...
			if ((not vsoPill("digest") and not vsoPill("softdigest")) or vsoPill("heal")) then
				if vsoTimer("speak") then

					vsoTimerSet("speak", 10, 20)
					vsoSay(vsoPick(bellyLines))
				end
			elseif ( vsoPill("digest") or vsoPill("softdigest") ) and vsoPill("laxative") then

				if vsoTimer("speak") and victimhealthpercent > 10 then

					vsoTimerSet("speak", 10, 20)
					vsoSay(vsoPick(disposalLines))
				end

				if (self.digestedtarget or victimhealthpercent < 10) then

					vsoSay(vsoPick(disposedLines))
					anim = "disposal";
					self.digestedtarget = true;
				end
			elseif (self.digestedtarget or victimhealthpercent < 70) and ( vsoPill("digest") or vsoPill("softdigest") ) then

				vsoSay(vsoPick(digestLines))
				vsoSound("belly1")
				anim = "bigtosmall";
			end
		...
		elseif anim == "microsquirmr" then
			anim = "fullmicro";
		elseif anim == "disposal" then
			anim = "disposalidle";
		elseif anim == "disposalidle" then
			anim = "erase";
		...
		if anim == "fullbig" then
			...
			elseif (movedir == 'J' or movedir == 'U') then
				...
				if vsoPill("antiescape") then
					wigglesneeded = wigglesneeded + 8 * ( vsoPillValue("antiescape") or 1.0 );
				end
	...
	if vsoPill("digest") or vsoPill("softdigest") or vsoPill("heal") then
		...
		if vsoPill("softdigest") then
			digestRate = digestRate + 0.5 *(vsoPillValue("softdigest") or 1.0);
		end
