-- This comes from an all-in-one lua! OV, digest, CV, digestcv, disposal, disposalcv, idle anims and commentary! UB can easily be included as well!
...
-- Randomly displays the "Player inside Pred" lines
  ...
  elseif world.loungeableOccupied(entity.id()) == true and eatingTimer <= 16 then -- Adjust eating timer based on swallow anim duration.
	...
	if animTimer >= 16 then
	  if spo_type == "cv" and animTimer <= 20 then
		animator.setAnimationState("bodyState", "nuttidle")
	  else
	    animPlaying = false
		nuttTimer = 0
	  end
	end
	if animPlaying == false and spo_type == "cv" then
	  nuttTimer = nuttTimer + 1
	end
	
    if animPlaying == false then 
	  if spo_type == "ov" and math.random(200) == 1 then  -- Random in belly anim
		animator.setAnimationState("bodyState", "safeburp")
		animator.playSound("burp")
		animTimer = 0
	    animPlaying = true
	  elseif spo_type == "cv" and nuttTimer >= 40 then -- On interval climax
		animator.setAnimationState("bodyState", "nutt")
		nuttTimer = 0
		animTimer = 0
	    animPlaying = true
	  else
